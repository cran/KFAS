test_that("importanceSSM works",{
  ...
  set.seed(1)
  expect_that(out <- KFS(model, nsim = 25, smoothing=c("mean","state","signal")),not(gives_warning()))
  ...
  set.seed(1)
  expect_that(out <- KFS(model, nsim = 25, filtering=c("mean","state","signal")),not(gives_warning()))
