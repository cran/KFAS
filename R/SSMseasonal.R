...
SSMseasonal <- 
  function(period, Q, sea.type = c("dummy", "trigonometric"), type, index, a1, P1, P1inf, 
    ...
    # k <- ((p-1)*(type != 2)+1)*((sea.type=='dummy') +
    # (sea.type=='trigonometric')*(period - 1))
    T <- matrix(0, m, m)
    Z <- matrix(0, p, m)
    if (type != 2) {
        for (i in 1:p) {
    ...
    if (missing(Q)) {
        ...
        if (sea.type == "dummy") {
            ...
            if (type == 1) {
                ...
                if (tvq == 1) {
                  for (i in 1:(tvq * (n - 1) + 1)) Qm[cbind(rep(1:(p * (period - 
                    1)), p), rep(1:(period - 1), p^2) + rep(0:(p - 1) * (period - 
                    1), each = p * (period - 1)), i)] <- rep(Q[, , i], each = (period - 
                    1))
                } else Qm[cbind(rep(1:(p * (period - 1)), p), rep(1:(period - 1), p^2) + 
                  rep(0:(p - 1) * (period - 1), each = p * (period - 1)), 1)] <- rep(Q, 
                  each = (period - 1))
            } else {
                if (length(Q) != 1 && (!identical(dim(Q)[1], dim(Q)[2]) || dim(Q)[1] != 
                  1 || !(max(dim(Q)[3], 1, na.rm = TRUE) %in% c(1, n)))) 
                ...
                Qm <- array(0, c(m, m, tvq * (n - 1) + 1))
                if (tvq == 1) {
                  for (i in 1:(tvq * (n - 1) + 1)) Qm[cbind(1:(period - 1), 1:(period - 
                    1), i)] <- Q[, , i]
