#' @rdname SSModel
#' @export
SSMseasonal <- function(period, sea.type = c("dummy", "trigonometric"), type, Q, index, a1, P1, P1inf,  n, ynames) {
    if (missing(index)) 
        index <- 1
    p <- length(index)
    ...
    if (sea.type == "dummy") {
        Z_univariate[1, 1] <- 1        
        state_names <- paste0(rep(paste0("sea_dummy", 1:(period - 1)), p), rep(ynames, each = period - 1))        
        T_univariate[1, ] <- -1
        ...
        Z_univariate[1, ] <- rep(c(1, 0), length.out = period - 1)
            ...
            state_names <- paste0(rep(c("sea_trig", "sea_trig*"), each = 1, length.out = (period - 1)), rep(1:floor(period/2), 
                each = 2, length.out = (period - 1)), rep(ynames, each = period - 1))
