kf <-
function (yt, Zt, Tt, Rt, Ht, Qt, a1, P1, P1inf = 0, 
    optcal = c(TRUE, TRUE, TRUE, TRUE), tol=1e-7) 
{
    if (!is.array(yt)) {
        if (!is.matrix(yt)) 
    ...
    kfout <- .Fortran("kf", PACKAGE = "KFAS", NAOK = TRUE,
        yt = y, ydimt = ydimt, tv = tv,
            Zt = Z, Tt = array(Tt, 
            c(m, m, (n - 1) * tv[1] + 1)), Rt = array(Rt, c(m, 
            r, (n - 1) * tv[2] + 1)), Ht = H, Qt = array(Qt, 
            c(r, r, (n - 1) * tv[3] + 1)), a1 = array(a1, c(m)), 
        P1 = array(P1, c(m, m)), at = at, Pt = Pt, vtuni = vtuni, 
        Ftuni = Ftuni, Ktuni = Ktuni, Pinf = Pinf, Pstar = Pstar, 
        Finfuni = Finfuni, Fstaruni = Fstaruni, Kinfuni = Kinfuni, 
        Kstaruni = Kstaruni, d = d, j = j, p = p, m = m, r = r, 
        n = n, lik = lik, optcal = optcal, info = info, vt = vt, 
        Ft = Ft, Kt = Kt, Lt = Lt, Finf = Finf, Fstar = Fstar, 
        Kinf = Kinf, Kstar = Kstar, Linf = Linf, Lstar = Lstar, tol=tol)
...
function (out) 
{
    ...
    storage.mode(H) <- "double"
    storage.mode(Z) <- "double"
    ks.out <- .Fortran("ks", PACKAGE = "KFAS", NAOK = TRUE, 
        out$ydimt, out$tv, Z = Z, array(out$Tt, c(out$m, out$m, 
            (out$n - 1) * out$tv[1] + 1)), H = H, array(out$Rt, 
            c(out$m, out$r, (out$n - 1) * out$tv[2] + 1)), array(out$Qt, 
        ...
        j = out$j, p = out$p, m = out$m, r = out$r, n = out$n, tol=out$tol)
...
forecast<-function(out,fc=1,Zt.fc=NULL,Tt.fc=NULL,Rt.fc=NULL, Ht.fc=NULL, Qt.fc=NULL)
{
...
if(is.null(Zt.fc)){
	if(!(is.na(dim(as.array(out$Zt))[3]) || dim(as.array(out$Zt))[3] == 1))
	{
...
if(is.null(Tt.fc)){
	if(!(is.na(dim(as.array(out$Tt))[3]) || dim(as.array(out$Tt))[3] == 1))
	{	
...
if(is.null(Rt.fc)){
	if(!(is.na(dim(as.array(out$Rt))[3]) || dim(as.array(out$Rt))[3] == 1))
	{
...
if(is.null(Ht.fc)){
	if(!(is.na(dim(as.array(out$Ht))[3]) || dim(as.array(out$Ht))[3] == 1))
	{
...
if(is.null(Qt.fc)){
	if(!(is.na(dim(as.array(out$Qt))[3]) || dim(as.array(out$Qt))[3] == 1))
	{	
...
for(i in 1:fc)
{
yt.fc[,i]<-matrix(Zt.fc[,,((i-1)*tv[1]+1)],out$p,out$m)%*%matrix(at.fc[,i],out$m,1)
Ft.fc[,,i]<-matrix(Zt.fc[,,((i-1)*tv[1]+1)],out$p,out$m)%*%Pt.fc[,,i]%*%t(matrix(Zt.fc[,,((i-1)*tv[1]+1)],out$p,out$m)) + Ht.fc[,,((i-1)*tv[4]+1)]
at.fc[,i+1]<-Tt.fc[,,((i-1)*tv[2]+1)]%*%matrix(at.fc[,i],out$m,1)
Pt.fc[,,i+1]<-Tt.fc[,,((i-1)*tv[2]+1)]%*%Pt.fc[,,i]%*%t(Tt.fc[,,((i-1)*tv[2]+1)]) + matrix(Rt.fc[,,((i-1)*tv[3]+1)],out$m,out$r)%*%Qt.fc[,,((i-1)*tv[5]+1)]%*%t(matrix(Rt.fc[,,((i-1)*tv[3]+1)],out$m,out$r))
}

return(list(yt.fc=yt.fc,Ft.fc=Ft.fc,at.fc=at.fc,Pt.fc=Pt.fc))
}

estmiss<-function(out)
{
Ft<-out$Ft
Fstar<-out$Fstar
Finf<-out$Finf
...
Zt<-array(out$Zt,c(out$p,out$m,(out$n-1)*tv[1]+1))
Ht<-array(out$Ht,c(out$p,out$p,(out$n-1)*tv[2]+1))
if(out$d>0){
for(i in 1:out$d)
{
if(out$ydimt[i]!=out$p)
{
Fstar[,,i]<- as.matrix(Zt[,,(i-1)*tv[1]+1])%*%out$Pstar[,,i]%*%t(as.matrix(Zt[,,(i-1)*tv[1]+1]))+Ht[,,(i-1)*tv[2]+1]
Finf[,,i]<- as.matrix(Zt[,,(i-1)*tv[1]+1])%*%out$Pinf[,,i]%*%t(as.matrix(Zt[,,(i-1)*tv[1]+1]))
}
}}
...
{
if(out$ydimt[i]!=out$p)
Ft[,,i]<-as.matrix(Zt[,,(i-1)*tv[1]+1])%*%out$Pt[,,i]%*%t(as.matrix(Zt[,,(i-1)*tv[1]+1]))+Ht[,,(i-1)*tv[2]+1]
}
}
...
if(!is.null(out$ahat))
{
	...
	{
		if(out$ydimt[i]!=out$p)
		yt[,i]<-as.matrix(Zt[,,(i-1)*tv[1]+1])%*%as.matrix(out$ahat[,i])
	}
est.out<-list(Fstar=Fstar, Finf=Finf, Ft=Ft, yt=yt)
}else{ est.out <- list(Fstar=Fstar, Finf=Finf, Ft=Ft)}
...
simsmoother <-
function (yt, Zt, Tt, Rt, Ht, Qt, a1, P1, P1inf=0, nnd, nsim=1, tol=1e-7) 
{
...
Qt<-array(Qt,c(r,r,(n - 1) * tv[3] + 1)) #toimiiko?
...
for (t in 1:n) {
    if (ydimt[t] > 0) {
       epsplus[1:ydimt[t], t,] <- rnorm(ydimt[t]*nsim, mean = 0, sd = 1)
       } }
...
sims.out <- .Fortran("simsmoother", PACKAGE = "KFAS", NAOK = TRUE, ydimt, tv, y, Z, Tt, Rt, H, Qt, a1, P1, P1inf, nnd, nsim, alphasim=alphasim, epsplus, etaplus, aplus1, p=p, n, m, r, info=info, tol)

if(sims.out$info!=0)
{
if(sims.out$info==1) stop("Couldn't compute Cholesky factorization of Ht!")
if(sims.out$info==2) stop("Couldn't compute Cholesky factorization of Qt!")
if(sims.out$info==3) stop("Couldn't compute Cholesky factorization of P1[1:nnd,1:nnd]!")
