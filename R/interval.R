...
interval <- function(model, interval = c("confidence", "prediction"), level, type = c("response", "link"), states = NULL, nsim, 
    se.fit = TRUE, timespan, prob = TRUE) {
    ...
    if (!identical(states, 1:attr(model, "m"))) # use only selected states
        imp$samples <- .Fortran(fzalpha, as.integer(dim(model$Z)[3] > 1), model$Z[, , if (dim(model$Z)[3] > 1) timespan else 1, 
            drop = FALSE], imp$samples[timespan, , drop = FALSE], signal = array(0, c(n2, p, nsim)), as.integer(p), n2, as.integer(m), nsim, as.integer(length(states)), 
            states)$signal
    ...
    if (type == "link") {
        ...
        for (j in 1:p) {
            imp$samples[timespan, j, ] <- switch(model$distribution[j], gaussian = imp$samples[timespan, j, ], poisson = (if (!prob) model$u[timespan, 
                j] else 1) * exp(imp$samples[timespan, j, ]), binomial = (if (!prob) model$u[timespan, j] else 1) * exp(imp$samples[timespan, 
