...
do t=1,n  
      ft(1:ydimt(t),1:ydimt(t),t)= ht(1:ydimt(t),1:ydimt(t),t) !in case optcal(2)=1
end do
   ...
   if(sum(hdiagtest)/=0) then 
      do t = 1, n
         if(hdiagtest(t)==1) then 
            info=0
            singtest: do i=1 , ydimt(t) !testing singularity
               if(ht(i,i,t)<=0) then
                  info=1
                  exit singtest
               end if
            ...
            if(info==1) then
               ...
               hdiagtest(t)=2
            else !LDL' decomposition, from Numerical Methods in Scientific computing, vol II, Dahlquist
               do k = 1, ydimt(t)
                  do i = k+1, ydimt(t)
      ...
      do t= 1, n
         if(ydimt(t)>0 .AND. hdiagtest(t)/=0) then
            if(hdiagtest(t)==1) then
               call dtrsv('l','n','u',ydimt(t),ht(1:ydimt(t),1:ydimt(t),t),ydimt(t),yt(1:ydimt(t),t),1) !solve y*=inv(L) * yt, 
            else
               call dgemv('t',ydimt(t),ydimt(t),1.0d0,evect(1:ydimt(t),1:ydimt(t)),ydimt(t),yt(1:ydimt(t),t),&
                    1,0.0d0,evalu(1:ydimt(t)),1)
               yt(1:ydimt(t),t) = evalu(1:ydimt(t)) 
            end if
         end if
     end do
  end if
...
if(maxval(abs(pinf(1:m,1:m,1))) > eps) then
   ...
   diffuse: do while(d < n)
      d = d+1
      do j=1, ydimt(d)
         call dsymv('u',m,1.0d0,psrec,m,zt(j,1:m,d),1,0.0d0,m1,1) 
         fstaruni(j,d) = ddot(m,zt(j,1:m,d),1,m1,1)  + ht(j,j,d)
         call dsymv('u',m,1.0d0,pirec,m,zt(j,1:m,d),1,0.0d0,m1,1)
         finfuni(j,d) = ddot(m,zt(j,1:m,d),1,m1,1)! finf
         vtuni(j,d) = yt(j,d) - ddot(m,zt(j,1:m,d),1,arec,1) !arec
         call dsymv('u',m,1.0d0,psrec,m,zt(j,1:m,d),1,0.0d0,kstaruni(1:m,j,d),1) ! kstar_t,i = pstar_t,i*t(z_t,i)
         call dsymv('u',m,1.0d0,pirec,m,zt(j,1:m,d),1,0.0d0,kinfuni(1:m,j,d),1) ! kinf_t,i = pinf_t,i*t(z_t,i)
...
!non-diffuse filtering begins
   prec = psrec
   do i = j+1, ydimt(d)     
      vtuni(i,d) = yt(i,d) - ddot(m,zt(i,1:m,d),1,arec,1) !vtuni         
      call dsymv('u',m,1.0d0,prec,m,zt(i,1:m,d),1,0.0d0,m1,1) ! p symmetric!
      ftuni(i,d) = ddot(m,zt(i,1:m,d),1,m1,1) + ht(i,i,d)
...
do t = d+1, n
   do i = 1, ydimt(t)     
      vtuni(i,t) = yt(i,t) - ddot(m,zt(i,1:m,t),1,arec,1) !univariate vt           
      call dsymv('u',m,1.0d0,prec,m,zt(i,1:m,t),1,0.0d0,m1,1) ! p symmetric!
      ftuni(i,t) = ddot(m,zt(i,1:m,t),1,m1,1)  + ht(i,i,t) !ftuni      
...
if(sum(hdiagtest)/=0 .AND. sum(optcal)>0) then
   do t = 1, n
      if(hdiagtest(t)/=0 .AND. ydimt(t)>0) then
         if(hdiagtest(t)==1) then
            call dtrmm('l','l','n','u',ydimt(t),m,1.0d0,ht(1:ydimt(t),1:ydimt(t),t),ydimt(t),zt(1:ydimt(t),1:m,t),ydimt(t))      
            call dtrmv('l','n','u',ydimt(t),ht(1:ydimt(t),1:ydimt(t),t),ydimt(t),yt(1:ydimt(t),t),1)
         else
            call dgemm('n','n',ydimt(t),m,ydimt(t),1.0d0,evect(1:ydimt(t),1:ydimt(t)),ydimt(t),zt(1:ydimt(t),1:m,t),&
                 ydimt(t),0.0d0,pm(1:ydimt(t),1:m),ydimt(t))
            zt(1:ydimt(t),1:m,t) = pm(1:ydimt(t),1:m)
            call dgemv('n',ydimt(t),ydimt(t),1.0d0,evect(1:ydimt(t),1:ydimt(t)),ydimt(t),yt(1:ydimt(t),t),&
                 1,0.0d0,evalu(1:ydimt(t)),1)
            yt(1:ydimt(t),t) = evalu(1:ydimt(t))
         end if
      end if
   ...
   do t=1, n
      if(ydimt(t)>0) then
         if(ydimt(t)==1) then
            ...
            call dcopy(ydimt(t),yt(1:ydimt(t),t),1,vt(1:ydimt(t),t),1)
            call dgemv('n',ydimt(t),m,-1.0d0,zt(1:ydimt(t),1:m,t),ydimt(t),at(1:m,t),1,1.0d0,vt(1:ydimt(t),t),1)
         end if
      end if
   end do
end if
   ...
   do t= 1, d
      if(ydimt(t)==p) then
         if(p==1) then
            ...
            fstar(1:ydimt(t),1:ydimt(t),t) = fstar(1:ydimt(t),1:ydimt(t),t) +  ft(1:ydimt(t),1:ydimt(t),t)
         end if
      end if
   end do
   do t=d+1, n
      if(ydimt(t)>0) then
         if(p==1) then
            ...
            call dgemm('n','t',ydimt(t),ydimt(t),m,1.0d0,pm(1:ydimt(t),1:m),ydimt(t),&
                 zt(1:ydimt(t),1:m,t),ydimt(t),1.0d0,ft(1:ydimt(t),1:ydimt(t),t),ydimt(t)) !ft, ht already in ft
         end if
      end if
   end do
end if
   ...
   do t=d+1, n
      if(ydimt(t)>0) then
         if(p==1) then
            ...
            call dgemm('n','t',m,ydimt(t),m,1.0d0,mm,m,zt(1:ydimt(t),1:m,t),ydimt(t),1.0d0,kt(1:m,1:ydimt(t),t),m) !TPZ'
...
if(optcal(4)==1 .AND. optcal(3)==1) then
   do t=1, d
      if(ydimt(t)>0) then
         if(maxval(abs(finf(1:ydimt(t),1:ydimt(t),t)))<eps) then !finf=0
   ...
   do t = d+1, n
      ...
      if(ydimt(t)>0) then
         call dgemm('n','n',m,m,ydimt(t),-1.0d0,kt(1:m,1:ydimt(t),t),m,zt(1:ydimt(t),1:m,t),ydimt(t),1.0d0,lt(1:m,1:m,t),m) !lt = t - kz
      end if
   end do
end if
