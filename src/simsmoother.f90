subroutine simsmoother( ymiss, ydimt, yna, tvh, tvz, tvhz, timevar, yt, zt, tt, rtv, ht, qt, a1, p1, p1pd, p1inf, &
  ...
  double precision, dimension(p,p,(n-1)*tvh+1) :: cholht
  ...
  do i = 1, floor(nsim/2.0d0)
     ...
     do t = 1, n       
        if(ydimt(t)>0) then
           call dtrmv('l','n','n',ydimt(t),cholht(1:ydimt(t),1:ydimt(t),(t-1)*tvh+1),ydimt(t),epsplus(1:ydimt(t),t,i),1)
        end if
        call dtrmv('l','n','n',r,cholqt(1:r,1:r,(t-1)*timevar(3)+1),r,etaplus(1:r,t,i),1)    
     end do
 ...
 if(mod(nsim,2) /= 0) then
    ...
    do t = 1, n       
       if(ydimt(t)>0) then
          call dtrmv('l','n','n',ydimt(t),cholht(1:ydimt(t),1:ydimt(t),(t-1)*tvh+1),ydimt(t),epsplus(1:ydimt(t),t,nsim),1)
       end if
       call dtrmv('l','n','n',r,cholqt(1:r,1:r,(t-1)*timevar(3)+1),r,etaplus(1:r,t,nsim),1)    
    end do
