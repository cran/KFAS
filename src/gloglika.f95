subroutine gloglika(yt, ymiss, timevar, zt, tt, rt, qt, a1, p1, p1inf,&
p, m, r, n, lik, tolf,rankp)
    ...
    double precision, dimension(m,m) :: pt,pinf,prec, psrec, pirec,im,mm
    ...
    if(maxval(p1inf) .GT.  0.0d0) then
        ...
        diffuse: do while(d .LT. n)
            ...
            do j=1, p
                ...
                if(ymiss(d,j).EQ.0) then
                    ...
                    call dsymv('u',m,1.0d0,psrec,m,zt(j,1:m,(d-1)*timevar(1)+1),1,0.0d0,kt(1:m,j),1) ! kt_t,i = pt_t,i*t(z_t,i)
                    ft(j) = ddot(m,zt(j,1:m,(d-1)*timevar(1)+1),1,kt(1:m,j),1)
                    ...
                    if (finf(j) .GT. tolf) then
                        ...
                        call dsyr('u',m,ft(j)/(finf(j)**2),kinf(1:m,j),1,psrec,m) !psrec = psrec +  kinf*kinf'*ft/finf^2
                        call dsyr2('u',m,-1.0d0/finf(j),kt(1:m,j),1,kinf(1:m,j),1,psrec,m) !psrec = psrec -(kt*kinf'+kinf*kt')/finf
                        ...
                        if (ft(j) .GT. 0.0d0) then
                            ...
                            call dsyr('u',m,(-1.0d0)/ft(j),kt(1:m,j),1,psrec,m) !psrec = psrec -kt*kt'/ft
            ...
            call dsymm('r','u',m,m,1.0d0,psrec,m,tt(1:m,1:m,(d-1)*timevar(3)+1),m,0.0d0,mm,m)
        ...
        do i = j+1, p
            if(ymiss(d,i).EQ.0) then
                ...
                ft(i) = ddot(m,zt(i,1:m,(d-1)*timevar(1)+1),1,kt(1:m,i),1)
    ...
    do t = d+1, n
        do i = 1, p
            if(ymiss(t,i).EQ.0) then
                ...
                ft(i) = ddot(m,zt(i,1:m,(t-1)*timevar(1)+1),1,kt(1:m,i),1)
                if (ft(i) .GT.  0.0d0) then !ft.NE.0
                    call daxpy(m,vt(i)/ft(i),kt(1:m,i),1,arec,1) !a_rec = a_rec + kt(:,i,t)*vt(:,t)
