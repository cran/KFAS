subroutine alphasim(ymiss,timevar, yt, zt, ht, tt, rtv, qt, a1, p1, &
p1inf, nnd,nsim2, epsplus, etaplus, aplus1, p, n, m, r, info,tolf,rankp,c,asim,tol,nd,ndl)
    !Subroutine for simulating state vectors of general state space model
    !Called by R function simState
    ...
    double precision, intent(inout), dimension(m,n,4*nsim2) :: asim
    ...
    do i = 1, nsim2
        ...
        do t = 1, n
            ...
            asim(1:m,t,i+nsim2) = ahat(1:m,t) + aplushat(1:m,t) - aplus(1:m,t)
            asim(1:m,t,i+2*nsim2) = ahat(1:m,t)+ c(i)*(asim(1:m,t,i)-ahat(1:m,t))
            asim(1:m,t,i+3*nsim2) = ahat(1:m,t)+ c(i)*(asim(1:m,t,i+nsim2)-ahat(1:m,t))
        end do
    end do


end subroutine alphasim
